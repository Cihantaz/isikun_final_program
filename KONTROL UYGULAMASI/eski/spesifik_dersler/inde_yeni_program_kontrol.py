...
# Kullanıcının verdiği yeni program
...
# Yeni INDE yerleri (kullanıcının tablosundan)
yeni_yerler = {
    'INDE3312': (1, 2),
    'INDE3314': (1, 1),
    'INDE4313': (1, 3),
...
print("=" * 100)
print("AYNI GUN/SLOT OLAN INDE DERSLERI")
print("=" * 100)

slot_dict = {}
for d, (g, s) in yeni_yerler.items():
    key = f"G{g}S{s}"
...
print()
print("=" * 100)
print("OGRENCI CAKISMALARI (CSV verisine gore - Tum program)")
print("=" * 100)

toplam_cakisma = 0
for d, (g, s) in sorted(yeni_yerler.items()):
    cakisan = []
    ...
    if cakisan:
        ...
        if len(cakisan) > 5:
            print(f"    ... ve {len(cakisan)-5} ders daha")
    else:
        print(f"{d} -> G{g}S{s} | CAKISMA YOK")

print()
print(f"TOPLAM CAKISAN OGRENCI: {toplam_cakisma}")

# DiffDay check
print()
print("=" * 100)
print("DIFFDAY KISIT KONTROLU (Yeni Program)")
print("=" * 100)
...
print()
print(f"TOPLAM IHLAL SAYISI: {ihlal_sayisi}")
